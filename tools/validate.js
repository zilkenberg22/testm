...
function emailValidate(email) {
    if (isNullOrUndefined(email)) return { error: true, message: "И-Майл хаягаа оруулна уу" };
    var re = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
    return { error: !re.test(email), message: "И-Майл хаягаа зөв оруулна уу" };
}

function userNameValidate(userName) {
    if (isNullOrUndefined(userName)) return { error: true, message: "Хэрэглэгчийн нэрээ оруулна уу" };
}

function passwordValidate(password) {
    var re = /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*])(?=.{8,})/;
    return { error: !re.test(password), message: "Нууц үг шаардлага хангаагүй байна" };
}
